...
@Component({
  selector: 'app-vacation-emergency',
  templateUrl: './vacation-emergency.component.html',
  styleUrls: ['./vacation-emergency.component.css']
})
export class VacationEmergencyComponent extends PersistAbstract<Vacation> implements OnInit {
  diasDisponiveis = 30;
  diasAbono = 10;
  mask: any = Mask;
  ...
  formInit(): FormGroup {
    const formGroup: FormGroup = this.formBuilder.group({
      ...
      dias_abono: this.formBuilder.control(0),
      justificativa_usuario: new FormControl('',
  ...
  sell(event: MatCheckboxChange) {
    ...
    this.form.controls.dias_abono.setValue(event.checked ? 10 : 0);
  }
  ...
  onSave() {
    ...
    let data = Object.assign({}, this.form.value) as VacationEmergencyComponent;
    data['id'] = this.dataId;
    data = this.transformBeforeSave(data);
    // data = this.form.value.justificativa_usuario;
    if (this.calcTotalDias() === 30) {
      this.isLoading = true;
      if (this.isUpdate) {
